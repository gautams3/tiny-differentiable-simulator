...
struct Laboratory {
  ...
  Laboratory(const std::string &shape_urdf_filename,
             ...
             VisualizerAPI *sim2) {
    auto &urdf_cache = get_cache<Scalar>();
    bool ignore_cache = true;
    
    ground =
        urdf_cache.construct(surface_urdf_filename, world_ground, sim2, sim, ignore_cache);
...
class PushEstimator
 ...
 public:
  ...
  PushEstimator() : CeresEstimator(0.0) {
    ...
    parameters[2] = {"v_transition", 0.01, 0.0001, 20000.};
  ...
  void add_training_dataset(const std::string &h5_filename, VisualizerAPI *sim,
    ...
    add_laboratory<double, DoubleUtils>(&labs_double, data, sim, sim2);
    add_laboratory<ADScalar, ADUtils>(&labs_ad, data, sim, sim2);
  }
  ...
  void rollout(const std::vector<ADScalar> &params,
    ...
    this->template rollout<ADScalar, ADUtils>(params, output_states,
                                              ADScalar(dt), ref_id);
  }
 ...
 private:
  ...
  static void add_laboratory(
    ...
    (*labs)[data.lab_name] = new Laboratory<Scalar, Utils>(
        shape_urdf_filename, surface_urdf_filename, tip_urdf_filename,
        exterior_filename, sim, sim2);
  }
 ...
 public:
  ...
  void rollout(const std::vector<Scalar> &params,
    ...
    for (std::size_t i = 0; i < data.time.size(); i += skip_steps) {
      ...
      if (i > 0) {
        tip->m_qd[0] = Scalar((data.tip_x[i] - data.tip_x[i - 1]) / data.dt);
        tip->m_qd[1] = Scalar((data.tip_y[i] - data.tip_y[i - 1]) / data.dt);
      }
      ...
      lab.tip_contact_model.resolveCollision({tip_contact}, dt);
      world_ground.step(dt);
      object->integrate(dt);
      object->forward_kinematics();

      output_states.push_back({object->m_q[0], object->m_q[1], object->m_q[3]});
...
int main(int argc, char *argv[]) {
  ...
  while (true) {
    ...
    std::vector<ADScalar> params{ADScalar(0.5), ADScalar(0.1), ADScalar(0.1)};
    estimator.template rollout<ADScalar, ADUtils>(params, output_states,
                                                    ADScalar(data.dt), 0, sim);
