...
constexpr std::size_t kTimesteps = 10;
...
void plot_trajectory(const T states) {
  ...
  for (int i = 0; i < state_dim; ++i) {
    std::vector<double> traj(kTimesteps);
...
template <typename T>
T rollout(const T* inputs, T* states, const bool doprint = false) {
  ...
  for (size_t i = 1; i < kTimesteps; i++) {
    ...
    const T v_prev = states[prev_idx + 1], x_prev = states[prev_idx + 0];
  ...
  T error_pos = pow(states[(kTimesteps-1) * state_dim + 0] - goal_position, 2.0);
  T error_vel = pow(states[(kTimesteps-1) * state_dim + 1] - goal_velocity, 2.0);
  T error = T(0.0);
  error += error_pos; 
...
void print_trajectory(const double *states, const double *inputs) {
...
struct CeresFunctional
{
  ...
  bool operator()(const  T* inputs, T* residual) const {
    T dummy_states[kTimesteps * state_dim];
    *residual = rollout<T>(inputs, dummy_states);
    return true;
  }
};
...
int main(int argc, char** argv) {
  ...
  double states[kTimesteps*state_dim];
  double inputs[kTimesteps];
