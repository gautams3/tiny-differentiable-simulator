...
void init_compound_pendulum(tds::MultiBody<Algebra>& mb,
                            ...
                            std::vector<typename Algebra::Scalar> masses = {})
{
  ...
  // TinySymmetricSpatialDyad, TinySpatialTransform
  // RigidBodyInertia, Transform
  ...
  for (int i = 0; i < num_links; i++) {
    ...
    l.X_T.translation.setValue(Algebra::zero(), pos_y, 
                               Algebra::zero());
    Scalar mass = static_cast<int>(masses.size()) > i
                  ? masses[i]
                  : Algebra::fraction(1, 1);
    Vector3 com;
    Scalar length = !link_lengths.empty() ? link_lengths[i] : Algebra::fraction(5,10);
    
    com.setValue(Algebra::zero(), length, Algebra::zero());
...
void double_pendulum_trajectory(
  typename Algebra::Scalar l1, typename Algebra::Scalar l2, typename Algebra::Scalar m1, typename Algebra::Scalar m2, typename Algebra::Scalar q1,
  typename Algebra::Scalar q2, typename Algebra::Scalar qd1, typename Algebra::Scalar qd2, typename Algebra::Scalar g, typename Algebra::Scalar dt,
  int steps, std::vector<typename Algebra::Scalar> *q1s, std::vector<typename Algebra::Scalar> *q2s,
  std::vector<typename Algebra::Scalar> *qd1s, std::vector<typename Algebra::Scalar> *qd2s
) {
  ...
  for (int t = 0; t < steps; ++t) {
    ...
    s12 = Algebra::sin1(q1 - q2);
    c12 = Algebra::cos1(q1 - q2);
    denom = two * m1 + m2 - m2 * Algebra::cos1(two * (q1 - q2));

    qdd1 = -g * (two * m1 + m2) * Algebra::sin1(q1) -
           m2 * g * Algebra::sin1(q1 - two * q2) -
           two * m2 * qd2 * qd2 * l2 * s12 -
           m2 * qd1 * qd1 * l1 * Algebra::sin1(two * (q1 - q2));
    qdd1 = qdd1 / (l1 * denom);

    qdd2 =
        two * s12 *
        (qd1 * qd1 * l1 * (m1 + m2) + g * (m1 + m2) * Algebra::cos1(q1) +
         qd2 * qd2 * l2 * m2 * c12);
    qdd2 = qdd2 / (l2 * denom);
